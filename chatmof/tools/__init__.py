...
from chatmof.tools.predict_property import _get_predict_properties
...
def load_mofchat_tools(llm: BaseLanguageModel) -> List[BaseTool]:
    tools = load_tools(_load_tool_names, llm=llm)
    custom_tools = [model() for model in _MOF_TOOLS.values()]
    custom_llm_tools = [model(llm=llm) for model in _MOF_LLM_TOOLS.values()]
